...
NOTICE:
It will be not possible to debug debugger if commands use package global variables. Because of second debugger instance use same ::Commands package and its variables. Think of this like second thread visit same sub while first thread is paused.
